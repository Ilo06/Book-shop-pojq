...
class BookCopyControllerIT extends FacadeIT {
  ...
  void setUp() {
    ...
    var genre = rest.postForEntity("/api/v1/genres", new CreateGenreDTO("ITGenre" + suffix),
        GenreResponse.class).getBody();
    var author = rest.postForEntity("/api/v1/authors", new CreateAuthorDTO("IT" + suffix, "Author"),
        AuthorResponse.class).getBody();
    var bookInput = new CreateBookDTO("Copy Test Book " + suffix, String.format("888%010d", System.nanoTime() % 10000000000L), "Desc",
  ...
  void listBookCopies_filtersByBookId() {
    ...
    ResponseEntity<List<BookCopyResponse>> byBook = rest.exchange(
        "/api/v1/book-copies?bookId=" + bookId, HttpMethod.GET, null,
    ...
    ResponseEntity<List<BookCopyResponse>> all = rest.exchange(
        "/api/v1/book-copies", HttpMethod.GET, null,
        new ParameterizedTypeReference<List<BookCopyResponse>>() {});
  ...
  void listBookCopies_filtersByStatus() {
    ...
    ResponseEntity<List<BookCopyResponse>> byStatus = rest.exchange(
        "/api/v1/book-copies?status=SOLD", HttpMethod.GET, null,
        new ParameterizedTypeReference<List<BookCopyResponse>>() {});
  ...
  void listBookCopies_filtersByBookIdAndStatus() {
    ...
    ResponseEntity<List<BookCopyResponse>> byBoth = rest.exchange(
        "/api/v1/book-copies?bookId=" + bookId + "&status=AVAILABLE", HttpMethod.GET, null,
        new ParameterizedTypeReference<List<BookCopyResponse>>() {});
